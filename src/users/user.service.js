...
export class UsersService {
    ...
    aduseraceess = async (adEmail, adEmailStatus, adVerifiCationToken) => {
        ...
        // if (!aduser) {
        //     throw new NotFoundError("어드민이 존재하지 않습니다.");
        // }
    ...
    signIn = async (email) => {
        ...
        let rating = await this.ordersRepository.ratingUserPoint(user.userId);

        const userpoint = rating[0] ? rating[0]._sum.totalPrice : 0;

        if (user.emailStatus !== "completion") {
            throw new ForbiddenError("활성화 되지 않은 유저입니다.");
        }
    ...
    getadmin = async () => {
        const aduser = await this.usersRepository.getadmin();
    ...
    aduserEdit = async (aduserId, adEmail, adPassword, adminName) => {
        ...
        const update = await this.usersRepository.aduserEdit(aduserId, adminName);
    ...
    aduserdelete = async (aduserId, adPassword) => {
        ...
        if (aduserId !== aduser.aduserId) {
            throw new NotFoundError("어드민 아이디가 일치하지 않습니다");
        }
        if (!aduser) {
            throw new NotFoundError("어드민이 존재하지 않습니다");
        }
        if (aduser.adEmailStatus !== "completion") {
            throw new NotFoundError("활성화 되지 않은 유저입니다.");
        }
        const passwordMatch = await bcrypt.compare(adPassword, aduser.adPassword);
        if (!passwordMatch) {
            throw new NotFoundError("현재 비밀번호가 일치하지 않습니다.");
        }
